...
ORDER BY IF(parentCommentId = 0, id, parentCommentId), parentCommentId, id"; */
 /*   
    $sql = "SELECT m.id,m.fromUser,m.sentDate,m.message,a.id as adId,a.title as adTitle,a.price,a.img,a.price,u.name FROM messages as m 
    INNER JOIN ads as a ON m.adId=a.id 
    INNER JOIN users as u ON u.id=a.userId 
    ORDER BY sentDate DESC"; // GROUP BY fromUser 
*/
$sql = "SELECT fromUser,sentDate,message FROM messages ORDER BY sentDate";
...
{ 
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        // other party
        $messages .= '<span class="msg_date">2 apr 11:58</span>';
        $messages .= '<span class="msg_message msg_other">Hej! Jag är intresserad. Har du de kvar?<br>Hälsningar<br>Dragan</span>';

        // me
        $messages .= '<span class="msg_date msg_r">3 apr 11:09</span>';
        $messages .= '<span class="msg_message msg_me">Hi,<br> Yes I still have them, but I have several people interested. I live in Lilla Essingen, If you are interested, would you be able to come look at them tomorrow evening? One guy is coming to see them today, but if he does not take them I can sell them to you.<br>-Ari</span>';
    }
    echo $messages;
